...
use unai::providers::{Provider, Together, TogetherModel};
use unai::Agent;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let model_options = ModelOptions {
        model: Some("meta-llama/Llama-3-8b-chat-hf".to_string()),
        provider: TogetherModel::default(),
        ..Default::default()
    };

    let client = Together::create_with_options(api_key, model_options, TransportOptions::default());
