...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut stream = agent.chat_stream(messages, vec![]);

    while let Some(chunk_result) = stream.next().await {
        match chunk_result {
            Ok(chunk) => {
                match chunk {
                    unai::model::StreamChunk::Data(msg) => {
                        for part in msg.parts() {
                            match part {
                                ...
                                Part::FunctionCall { name, arguments, .. } => {
                                    println!("\n[Tool Call: {} args={}]", name, arguments);
                                }
                                Part::FunctionResponse { name, response, .. } => {
                                    println!("\n[Tool Result: {} response={}]", name, response);
                                }
                                _ => {}
                            }
                        }
                    }
                    unai::model::StreamChunk::Usage(usage) => {
                        println!("\n[Usage: {:?}]", usage);
                    }
                    unai::model::StreamChunk::Finish(reason) => {
                        println!("\n[Finish: {:?}]", reason);
                    }
                }
            }
